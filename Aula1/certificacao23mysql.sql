...
sigla_cert CHAR(5) NOT NULL REFERENCES certificacao
...
( num_matricula integer NOT NULL REFERENCES aluno (num_matricula)  ON DELETE CASCADE ON UPDATE CASCADE,
  sigla_modulo CHAR(5) NOT NULL REFERENCES modulo (sigla_modulo)  ON DELETE CASCADE ON UPDATE CASCADE ,
...
ALTER TABLE instrutor MODIFY nome_instrutor CHAR(35) ;
...
ALTER TABLE aluno_certificacao CHANGE dt_matricula dt_matricula TIMESTAMP ; 

-- renomeando uma tabela
ALTER TABLE instrutor_certificacao RENAME instrutor_certificacao;

-- check em situação da matricula
DROP TRIGGER check_aluno_matr ; 
DELIMITER $$
CREATE TRIGGER check_aluno_matr BEFORE INSERT ON aluno_certificacao
FOR EACH ROW
BEGIN
    IF NEW.situacao_matricula NOT IN ('CURSANDO', 'CONCLUIDO', 'CANCELADA', 'TRANCADA') THEN
         SIGNAL SQLSTATE '45000'
         SET MESSAGE_TEXT = 'Valor não permitido para situação do aluno na certificação';
    END IF;
END;
$$   
...
UPDATE aluno SET num_matricula = 1001
WHERE num_matricula = 18 ;

COMMIT ;

INSERT INTO instrutor_modulo VALUES (19, 'ICND2', current_date - INTERVAL  '8' DAY, null);
