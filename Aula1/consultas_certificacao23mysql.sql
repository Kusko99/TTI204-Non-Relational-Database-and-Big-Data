...
-- 3 - funções de data
SELECT NOW(), curdate(), curtime(), current_date ;
...
-- Interval
SELECT curDate() + INTERVAL '100' DAY , curDate() + INTERVAL '3' YEAR;
...
AND am.resultado_final = 'Aprovado' ;
...
SELECT a.nome_aluno, m.nome_modulo
FROM aluno a INNER JOIN aluno_modulo am
ON a.num_matricula = am.num_matricula 
INNER JOIN modulo m
ON am.sigla_modulo = m.sigla_modulo
WHERE UPPER(am.resultado_final) LIKE 'APROV%' ;
...
AND am.sigla_modulo IN (
                         ...
                         FROM aluno a, aluno_modulo am
                         WHERE a.num_matricula = am.num_matricula
                         AND UPPER(a.nome_aluno) LIKE 'MARIA SILVA%' ) ;
...
WHERE m.carga_horaria_modulo IN (
